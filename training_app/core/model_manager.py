...
"""
    ...
    def _convert_anomalib_model_to_onnx(self, model_file: Path, onnx_path: Path) -> bool:
        """
        anomalib PyTorchモデルをONNXに変換（根本問題解決版）
        ...
        try:
            ...
            # anomalibモデルの根本問題を解決したONNX変換
            self.logger.info("anomalibモデルの根本問題を解決したONNX変換を実行")
            
            # anomalibモデルをONNX対応に修正したラッパーモデルを作成
            ...
            if not conversion_success:
                self.logger.error("根本問題解決版ONNX変換でも失敗しました")
                return False
    ...
    def _convert_anomalib_checkpoint_to_onnx(self, checkpoint_file: Path, onnx_path: Path) -> bool:
        """
        anomalib Lightning checkpointをONNXに変換（根本問題解決版）
        ...
        try:
            ...
            if not conversion_success:
                self.logger.error("Checkpoint根本問題解決版ONNX変換でも失敗しました")
                return False
    ...
    def _create_onnx_compatible_anomalib_model(self, original_model, model_config):
        """ONNX対応のanomalibモデルラッパーを作成（根本問題解決 - 直接的アプローチ）"""
        try:
            import torch
            import torch.nn as nn
            import torch.nn.functional as F
            
            self.logger.info("直接的ONNX対応anomalibモデル作成開始")
            
            class DirectONNXAnomalibModel(nn.Module):
                """INDICES境界エラーを直接解決したanomalibモデル"""
            ...
            self.logger.info("直接的ONNX対応anomalibモデル作成完了")
            return onnx_model
